...
\begin{definition}\label{defn:extended-sharp}
  Suppose \(B\) is the colimit of a sequential diagram\viktor{is this the same
    as considering \enquote{discrete formally smooth ind-schemes} as in
    \parencite{arxiv/math/0107143}?}
  \[
...
\(f\) to be \(\Tr{f/S} = \pi_\#(\Tr{f/B})\colon E/S\to B/S\).\viktor{prove that
  \(\pi_\#(\one{B}) = B/S\)}
...
\begin{lemma}\label{lem:transfer-natural}
  ...
  \(E'/B'\) with respect to their respective diagonals.\viktor{meh} We obtain a
  diagram\viktor{check this diagram looks like it should w/ 6 operations}
  \[
    \begin{tikzcd}
